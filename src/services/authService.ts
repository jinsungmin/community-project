...
async function getAccountIdFromToken(type: 'kakao' | 'google', token: string): Promise<any> {
    try {
        ...
        switch (type) {
            case 'kakao': {
                const {id, email} = await Kakao.getMe(token)
                result.accountId = id
                result.email = email
                break
            }
...
export {userSignUp, userSignIn, socialSignIn, userUpdatePassword, userResetPassword, userRefreshToken, adminSignIn, adminRefreshToken}
