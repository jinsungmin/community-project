import * as AccountService from './accountService'
import * as AdministratorService from './administratorService'
import * as AuthService from './authService'
import * as CategoryService from './categoryService'
import * as CommentService from './commentService'
import * as NewsService from './newsService'
import * as PostService from './postService'
import * as RatingService from './ratingService'
import * as UserService from './userService'
import * as VerificationService from './verificationService'

export {AccountService, AdministratorService, AuthService, NewsService, CategoryService, CommentService, PostService, RatingService, UserService, VerificationService}
