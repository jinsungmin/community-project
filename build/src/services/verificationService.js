...
exports.createEmail = exports.confirm = exports.findOne = exports.create = void 0;
const models_1 = require("../models");
const jwt_1 = require("../libs/jwt");
const mailer_1 = require("../loaders/mailer");
async function create(options) {
    try {
        ...
        await mailer_1.sendVerifyEmail(email, code); // 인증 코드 발송
        return ret;
    }
    catch (e) {
...
async function createEmail(options) {
    try {
        const { type, email } = options;
        if (type === 'reset') {
            const account = await models_1.Account.findOne({ accountId: email });
            const user = await models_1.User.findOne({ id: account.userId });
            if (!user)
                throw new Error('not_found');
        }
        if (type === 'register') {
            const verification = await models_1.Verification.findOne({ type, email });
        ...
        const expireAt = new Date(exp * 1000);
        const codeToken = await jwt_1.createToken({ sub: id.toString(), exp }, { algorithm: 'RS256' });
        const ret = { email, code, codeToken, expireAt };
        await mailer_1.sendVerifyEmail(email, code); // 인증 코드 발송
        /*
        if (process.env.NODE_ENV !== 'production') {
          ret.code = code
